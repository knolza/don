...
int nAnonymizeGamblrAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Gamblr
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Gamblr
    // Mac: ~/Library/Application Support/Gamblr
    // Unix: ~/.gamblr
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Gamblr";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "gamblr.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty gamblr.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override gamblr.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "gamblrd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Gamblr will not work properly.");
