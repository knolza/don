...
static const int64_t nTargetTimespan = 24 * 60 * 60; // Gamblr: 1 day
static const int64_t nTargetSpacing = 2.5 * 60; // Gamblr: 2.5 minutes
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, gamblr - DarkGravity v3, written by Evan Duffield - evan@gamblrpay.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        ...
        if (retarget == DIFF_BTC)
        {
            ...
            // Gamblr: This fixes an issue where a 51% attack can change difficulty at will.
