...
bool CMasternodeConfig::read(std::string& strErr) {
    ...
    for(std::string line; std::getline(streamConfig, line); )
    {
        ...
        if (!(iss >> alias >> ip >> privKey >> txHash >> outputIndex >> donation)) {
            ...
            if (!address.IsValid()) {
                strErr = "Invalid Gamblr address in masternode.conf line: " + line;
                streamConfig.close();
                return false;
            }
        }
        ...
        if(Params().NetworkID() == CChainParams::MAIN){
            if(CService(ip).GetPort() != 7777) {
                strErr = "Invalid port detected in masternode.conf: " + line + " (must be 7777 for mainnet)";
                streamConfig.close();
                return false;
            }
        } else if(CService(ip).GetPort() == 7777) {
            strErr = "Invalid port detected in masternode.conf: " + line + " (7777 must be only on mainnet)";
            streamConfig.close();
            return false;
        }
